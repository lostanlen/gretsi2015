...
\blfootnote{Ce travail est financ\'{e} par la bourse ERC InvariantClass 32095.
...
\subsection{Transformée de scattering temporelle}
...
\begin{enumerate}
\item $\Vert\ddot{\beta}/\dot{\beta}\Vert_{\infty}\ll\lambda_{1}\eta/Q$
(filtre lentement variable),
\item $\Vert\dot{\hat{h}}/\hat{h}\Vert_{\infty}\Vert1/\dot{\beta}\Vert_{\infty}\ll Q\eta^{-1}/\lambda_{1}$
(r\'{e}ponse fr\'{e}quentielle r\'{e}guli\`{e}re),
\item $\Vert\ddot{\alpha}/\dot{\alpha}\Vert_{\infty}\ll\lambda_{1}\eta/Q$
...
toute d\'{e}viation de chroma $\Delta$ relative à $\log_{2} \dot{\alpha}$. Ce r\'{e}sultat sugg\`{e}re
...
\begin{figure}[fig2]
    ...
    \end{center}
    \legende{Deux ondelettes en spirale $\psi_{\lambda_{2}}(u_{2})$ \'{e}tal\'{e}es sur le plan temps-fr\'{e}quence, pr\'{e}sentant des $\lambda_{2}=(\alpha,\beta,\gamma)$ diff\'{e}rents et une localisation diff\'{e}rente sur le scalogramme. \`{A} gauche : $\gamma^{-1}=120\,\mathrm{ms}$, $\beta^{-1}=-0.25\,\mathrm{octave}$, $\gamma^{-1}=+2\ \mathrm{octaves}$. \`{A} droite : $\alpha^{-1}=60\,\mathrm{ms}$, $\beta^{-1}=+0.5\,\mathrm{octave}$, $\gamma^{-1}=-4\,\mathrm{octaves}$. On a affich\'{e} la partie r\'{e}elle des coefficients. Le noir correspond \`{a} des coefficients positifs et le blanc \`{a} des coefficients n\'{e}gatifs.}
\end{figure}
